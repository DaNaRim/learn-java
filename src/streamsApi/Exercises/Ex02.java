...
public class Ex02 {
    ...
    public static void main(String[] args) {
        //Exercise 2 — Obtain a list of order with products belong to category “furniture”

        List<Order> ordersWithFurniture = orders.stream()
                .filter(order -> order.getProducts().stream()
                        .anyMatch(product -> product.getCategory().equals("furniture")))
                .collect(Collectors.toList());

        ordersWithFurniture.forEach(order ->
